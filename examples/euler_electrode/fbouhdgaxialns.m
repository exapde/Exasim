function fb = fbouhdgaxialns(u, q, w, v, x, t, mu, eta, uhat, n, tau)
    ...
    p = r*T;
    H = E + T;
    % don't forget to change T to w(1) in all the other places!
    % don't forget to check if this value for the enthalpy is really the correct one in the riemann invariant
    ...
    % f_sym(1) = ry + tau*(u(1) - uhat(1));  % dr/dy = 0
    % f_sym(2) = ny*(u(2) - uhat(2)) - nx*(u(3) - uhat(3)); % zero velocity component in y
    % f_sym(3) = 0.0  - nx*uhat(2) - ny*uhat(3); % zero velocity component in x
