...
L = 10;
pde.physicsparam = [L, 1];       % unit thermal conductivity
pde.linearsolvertol = 1e-7/L; % GMRES tolerance
%mesh.p(1,:) = L*mesh.p(1,:);

pde.tau = 2*L*L;              % DG stabilization parameter
...
figure(1); clf; scaplot(mesh0,sol0(:,1,:),[],2); axis on; axis equal; axis tight;
figure(2); clf; scaplot(mesh0,sol(:,1,:),[],2); axis on; axis equal; axis tight;
figure(3); clf; scaplot(mesh,sol0(:,1,:),[],2); axis on; axis equal; axis tight;
figure(4); clf; scaplot(mesh,sol(:,1,:),[],2); axis on; axis equal; axis tight;