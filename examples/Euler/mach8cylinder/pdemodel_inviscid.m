...
function f = flux(u, q, w, v, x, t, mu, eta)
    %f = getfluxav2d_laplacian(u,q,v,mu);
    f = getfluxav2d_inviscid(u,q,v,mu);
    f = reshape(f,[4,2]);        
end

function f = avfield(u, q, w, v, x, t, mu, eta)
    %f = sensorLaplacian(u,q,v,mu);
...
%     f = flux(uhat, q, w, v, x, t, mu, eta); ma17 cylinder used f(uhat)
    ...
    fsw = flux(usw, q, w, v, x, t, mu, eta);
    fhat = 0.5*(f + fsw);
    faw = fhat(:,1)*n(1) + fhat(:,2)*n(2) + tau*(u-uhat);    
    %faw = fi;
