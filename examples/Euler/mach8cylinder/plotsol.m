...
% Oout = getsolution(['dataout/out_outputCG_t' num2str(ti)],dmd,master.npe);

% sb_out = Oout(:,1,:);
% out = eulereval(Uout,'rE',gam,Minf);
% out = Uout(:,12,:);
out = Uout(:,4,:);
symcheck = Uout(:,9,:);
max(symcheck(:))/min(symcheck(:))
[om,ind] = max(abs(out(:)));
figure(1); clf; scaplot(mesh,out,[],1,1);colormap('jet');

% x = mesh.dgnodes(:,1,:);
% y = mesh.dgnodes(:,2,:);
% hold on;
% plot(x(ind),y(ind),'o');
% figure(2); clf;colormap('jet'); scaplot(mesh, sb_out, [],1);
% 
% colormap("jet")
%%
% Uout = getsolution(['da%%taout/out_odg' num2str(ti)],dmd,master.npe);
