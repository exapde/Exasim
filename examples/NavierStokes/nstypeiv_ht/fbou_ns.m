function [fh,fh_udg,fh_uh] = fbou_ns(ib,uinf,nl,p,udg,uh,param,time)
%FHAT flux function
...
switch ib
    ...
    case 2  % Adiabatic Wall     
        ...
        [fh4,fh4_udg,fh4_uh] = fhat3(nl,p,udg,uh,param,time);
    ...
    case 3 % Isothermal wall
        %[fh,fh_udg,fh_uh] = fhat_ns(nl,p,udg,uh,param,time);
        %fh_u = fh_udg(:,:,1:nch);
    ...
    case 8 % Isothermal wall
        ...
        fh_uh(:,2,2) = -1;
        fh_uh(:,3,3) = -1;
        fh_uh(:,4,:) = [-TisoW*ones(ng,1), zero, zero, ones(ng,1)];        
        fh_udg = cat(3,fh_u,reshape(fh_q,ng,nch,2*nch)); 
    case 9
        gam  = 1.4;
        ma   = 8.03;
        ...
        ua   = 1;
        va   = 0;
        pa   = ra*(ua^2+va^2)/(gam*ma^2);
        ea   = pa/(gam-1) + 0.5*ra*(ua^2+va^2);

        mb   = 5.25;
        rb   = 1.99e1/5.97;
        ub   = 5326.9/5577.7*cos(12.5*pi/180);
        vb   = 5326.9/5577.7*sin(12.5*pi/180);
        pb   = rb*(ub^2+vb^2)/(gam*mb^2);
        eb   = pb/(gam-1) + 0.5*rb*(ub^2+vb^2);

        Ua = [ra ra*ua ra*va ea];
        Ub = [rb rb*ub rb*vb eb];
        ui = u;
        kk = param{end-1};
        %kk = 50;
        df = 0.5*(1 + tanh(-kk*(p(:,2)-0.3271*p(:,1)-0.4147)));
        for i = 1:nch
          ui(:,i) = Ua(i) + df*(Ub(i)-Ua(i));
        end        
        fh = ui - uh;                
        fh_udg = zeros(ng,nch,nc);       
        fh_uh = zeros(ng,nch,nch);
...
%         uinf(:,3) = uinf(:,3) - nl(:,2).*un;
%         
%         [fh4,fh4_udg,fh4_uh] = fhat_ns(nl,p,udg,uh,param,time);
%         
%         r    = udg(:,1);
%         ru   = udg(:,2);
%         rv   = udg(:,3);
