...
mesh.boundaryexpr = {@(p) (p(2,:)<yO + 1e-6 & p(1,:)<xO + 1e-6),         ... % 1 Slip wall
  @(p) (p(1,:)<xL + 1e-6) &  sqrt(p(1,:).^2+p(2,:).^2)>2*rnose+1e-6,                           ... % 2 Inflow (cap arc)
  @(p) (p(1,:)<xL + 1e-6) &  sqrt(p(1,:).^2+p(2,:).^2)<2*rnose+1e-6,                           ... % 3 cap
  @(p) (p(2,:)<yL + 1e-6 & p(1,:)>xL - 1e-6 & p(1,:)<xC + 1e-6), ... % 4 Flat plate
  @(p) (p(2,:)<yE + 1e-6 & p(1,:)>xC - 1e-6),         ... % 5 Flared cone
  @(p) (p(1,:)>xT - 1e-6),  ... % 6 Outflow
  @(p) (p(1,:) > -1e10)                                          ... % 7 Freestream
};
