function mesh = mkmesh_sphericalfrustum(slant, porder, n, L, dz)
...
if nargin < 4 || isempty(L)
  ...
  points = sphericalfrustum(mesh2d.p, slant, L);
end
...
if nargin < 4 || isempty(L)
  ...
  points = sphericalfrustum(p, dz, L);
end
...
function points = sphericalfrustum(p, slant, L)

% distances to the first point
D = slant-slant(:,1);
D = sqrt(sum(D.^2, 1));
...
for i = 1:ns
  ...
  h = sqrt(l^2 - r^2);     % the height of the cone whose base is radius a  
  m = sqrt(a.^2 + h^2);    % slant height of the inner cone whose base is radius a
  s = (h*l)./m;            % the height of the projected cone whose base is radius b
  points(2,:,i) = y; % y-coordinate of points on the projected circle of radius b
  points(3,:,i) = z; % z-coordinate of points on the projected circle of radius b
