function [An,Anm] = getan2d(nl,m,param)
%FHAT flux function
...
a = 100;
rlam1   = tanh(a*(un+c));
rlam1m1 = -a*(tanh(a*(un+c)).^2 - 1).*(unm1+cm1);
rlam1m2 = -a*(tanh(a*(un+c)).^2 - 1).*(unm2+cm2);
rlam1m3 = -a*(tanh(a*(un+c)).^2 - 1).*(unm3+cm3);
rlam1m4 = -a*(tanh(a*(un+c)).^2 - 1).*(unm4+cm4);    

rlam2   = tanh(a*(un-c));
rlam2m1 = -a*(tanh(a*(un-c)).^2 - 1).*(unm1-cm1);
rlam2m2 = -a*(tanh(a*(un-c)).^2 - 1).*(unm2-cm2);
rlam2m3 = -a*(tanh(a*(un-c)).^2 - 1).*(unm3-cm3);
rlam2m4 = -a*(tanh(a*(un-c)).^2 - 1).*(unm4-cm4);

rlam3   = tanh(a*un);
rlam3m1 = -a*(tanh(a*(un)).^2 - 1).*unm1;
rlam3m2 = -a*(tanh(a*(un)).^2 - 1).*unm2;
rlam3m3 = -a*(tanh(a*(un)).^2 - 1).*unm3;
rlam3m4 = -a*(tanh(a*(un)).^2 - 1).*unm4;
s1      = 0.5*(rlam1   + rlam2);
