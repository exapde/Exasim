...
mesh.boundaryexpr = {
  @(p) (p(1,:) < xmin + 1e-6), ... %  farfield 
  @(p) (p(2,:) < ymin + 1e-6 && p(1,:) < 0.0 + 1e-6), ... %  farfield  
  @(p) (p(2,:) < ymin + 1e-6 && p(1,:) > 0.0 - 1e-6), ... %  Wall  
  @(p) (p(1,:) > xmax - 1e-6),  ...%  Outflow
  @(p) (p(2,:) > ymax - 1e-6)         %  farfield
};
...
figure(1);clf;boundaryplot(mesh,1); hold on; %farfield
boundaryplot(mesh,2); %farfield
boundaryplot(mesh,3); %wall
boundaryplot(mesh,4); %outflow
boundaryplot(mesh,5); %farfield 
