function gencodeelem3(filename::String, f, xdg, uinf, param)
...
tmp = tmp * "(T *f, T *xdg, T *uinf, T *param, int ng, int ncx, int nce, int npe, int ne)\n";
tmp = tmp * "{\n";
tmp = tmp * "\tint blockDim = 256;\n";
tmp = tmp * "\tint gridDim = (ng + blockDim - 1) / blockDim;\n";
tmp = tmp * "\tgridDim = (gridDim>1024)? 1024 : gridDim;\n";
tmp = tmp * "\tkernel" * gpufile * "<<<gridDim, blockDim>>>(f, xdg, uinf, param, ng, ncx, nce, npe, ne);\n";
tmp = tmp * "}\n\n";
