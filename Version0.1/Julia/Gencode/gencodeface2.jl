function gencodeface2(filename::String, f, xdg, udg1, udg2, odg1, odg2, wdg1, wdg2, uhg, nlg, tau, uinf, param, time)
...
tmp = "(T *f, T *xdg, T *udg1, T *udg2,  T *odg1, T *odg2,  T *wdg1, T *wdg2,  T *uhg, T *nlg, T *tau, T *uinf, T *param, T time, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw)\n";
...
str = varsassign(str, "tau", length(tau), 0);
str = varsassign(str, "xdg", length(xdg), 1);
str = varsassign(str, "udg1", length(udg1), 1);
str = varsassign(str, "udg2", length(udg2), 1);
str = varsassign(str, "uhg", length(uhg), 1);
str = varsassign(str, "odg1", length(odg1), 1);
str = varsassign(str, "odg2", length(odg2), 1);
str = varsassign(str, "wdg1", length(wdg1), 1);
str = varsassign(str, "wdg2", length(wdg2), 1);
str = varsassign(str, "nlg", length(nlg), 1);
str = sympyassign(str, f);

stropu = stropu * str * "\t}\n" * "}\n\n";
tmp = "template void " * opufile;
tmp = tmp * "(double *, double *, double *, double *, double *, double *, double *, double *, double *, double *, double *, double *, double *, double, int, int, int, int, int, int, int);\n";
...
tmp = tmp * "(T *f, T *xdg, T *udg1, T *udg2,  T *odg1, T *odg2,  T *wdg1, T *wdg2, T *uhg, T *nlg, T *tau, T *uinf, T *param, T time, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw)\n";
tmp = tmp * "{\n";
...
tmp = tmp * "template void " * gpufile;
tmp = tmp * "(float *, float *, float *, float *, float *, float *, float *, float *, float *, float *, float *, float *, float *, float time, int, int, int, int, int, int, int);\n";
strgpu = strgpu * tmp;
