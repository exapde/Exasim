...
static void KokkosVisTensorsTemplate(dstype* f, const dstype* xdg,
                                     ...
                                     const int ncw_runtime)
{
    ...
    Kokkos::parallel_for("VisTensors", ng, KOKKOS_LAMBDA(const size_t i) {
        ...
        Model::vis_tensors(out_local, x, uq, v, w, param, uinf, time);

        for (int k = 0; k < nten_runtime; ++k) f[k * ng + i] = out_local[k];
    });
}
...
{
    KokkosVisTensorsTemplate<PdeModel>(f, xdg, udg, odg, wdg, uinf, param,
