...
static void KokkosOutputTemplate(dstype* f, const dstype* xdg,
                                 ...
                                 const int ne)
{
    ...
    Kokkos::parallel_for("Output", ng, KOKKOS_LAMBDA(const size_t i) {
        dstype x[nd];
        dstype uq[nc];
        ...
        Model::output(out_local, x, uq, v, w, param, uinf, time);

        for (int k = 0; k < nce_runtime; ++k) f[k * ng + i] = out_local[k];
    });
}

void KokkosOutput(dstype* f, const dstype* xdg, const dstype* udg, const dstype* odg, const dstype* wdg, const dstype* uinf, const dstype* param, const dstype time, const int modelnumber, const int ng, const int nc, const int ncu, const int nd, const int ncx, const int nco, const int ncw, const int nce, const int npe, const int ne)
{
    KokkosOutputTemplate<PdeModel>(f, xdg, udg, odg, wdg, uinf, param, time,
