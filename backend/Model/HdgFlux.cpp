void HdgFlux(dstype* f, dstype* J1, dstype* J2, const dstype* x, const dstype* uq, const dstype* v, const dstype* w, const dstype* eta, const dstype* mu, const dstype t, const int modelnumber, const int N, const int szx, const int szuq, const int szv, const int szw, const int szeta, const int szmu)
{
  ...
  Kokkos::parallel_for("Flux", N, KOKKOS_LAMBDA(const size_t i) {
    ...
    dstype x73 = uq4*x72;
    dstype x74 = x0*uq0;
    dstype x75 = x69*x32*x17;
    dstype x76 = x26 - x50;
    dstype x77 = -2*x41 + 2*x52 + x76;
    dstype x78 = uq8*x72;
    dstype x79 = uq4*x30;
    dstype x80 = 1.0*x37;
    dstype x81 = uq1*x80;
    ...
    dstype x100 = x56*x17;
    dstype x101 = x55*x17;
    dstype x102 = x33*(-x8 + x74*(x58 - (-x55 - x56)*uq0));
    dstype x103 = x61*x18;
    ...
    J1[2 * N + i] = x66;
    J1[3 * N + i] = uq1*x67 + x33*(x28 - uq4*x60 + x74*(-x25 - x27 - uq4*x57 - (x1*x73 + x4*x73 - x70*x14 - x71*x20)*uq0)) - x61*x19 + x65*x10 - x68*x22 - x75*x29 + uq1*x54*x35;
    J1[4 * N + i] = 0;
    J1[5 * N + i] = x66;
    J1[6 * N + i] = -x3 + x60 + x77*x18;
    J1[7 * N + i] = uq2*x67 + x33*(x45 - uq8*x60 + x74*(-x42 - x44 - uq8*x57 - (x1*x78 + x4*x78 - x70*x21 - x71*x11)*uq0)) - x61*x22 + x65*x13 - x68*x40 - x75*x46 + uq2*x77*x35;
    J1[8 * N + i] = 1;
    J1[9 * N + i] = 2*x13 - 1.33333333333333*x79 + x82;
    J1[10 * N + i] = x84;
    J1[11 * N + i] = x36 + x38 - 1.33333333333333*x86 - x87 + x33*(uq4*x81 + x74*(-uq0*x76 - x88*x61)) - x5*x85;
    J1[12 * N + i] = 0;
    J1[13 * N + i] = x84;
    J1[14 * N + i] = x82 + uq4*x89;
    J1[15 * N + i] = x91 + x33*(uq8*x81 + x74*(-x61*x90 - (x43 - x63)*uq0)) - x63*x17 + x64*x18;
    J1[16 * N + i] = 0;
    J1[17 * N + i] = x93 + uq8*x89;
    J1[18 * N + i] = x94;
    J1[19 * N + i] = x91 + x33*(uq4*x92 + x74*(-x88*x68 - (x24 - x64)*uq0)) + x63*x18 - x64*x17;
    J1[20 * N + i] = 1;
    J1[21 * N + i] = x94;
    J1[22 * N + i] = 2*x10 - 1.33333333333333*x83 + x93;
    J1[23 * N + i] = x38 + x47 - x86 - 1.33333333333333*x87 - x3*x85 + x33*(uq8*x92 + x74*(-uq0*x53 - x68*x90));
    J1[24 * N + i] = 0;
    J1[25 * N + i] = x0;
    J1[26 * N + i] = 0;
