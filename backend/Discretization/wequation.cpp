...
// void extendedwEquation(dstype *wdg, dstype *xdg, dstype *udg, dstype *odg, dstype *wsrc, 
//       dstype *tempg, appstruct &app, commonstruct &common, Int ng, Int backend)
// {        
//     Int ncu = common.ncu; // number of compoments of (u)
//     Int nd = common.nd; // spatial dimension
...
//       int ncs = ncw/(1+ncu);        
//       dstype *s = tempg; // temporary array    
//       dstype *s_wdg = &tempg[ng*ncs]; // temporary array 
//       dstype *dwdu = &tempg[ng*ncs + ng*ncs*ncs]; // temporary array 
//       // use Newton to solve the nonlinear system  alpha * dw/dt + beta w = S(w, u, q) to obtain w for given (u, q)             
//       for (int iter=0; iter<20; iter++) {
//         // alpha * dw/dt + beta w = sourcew(u,q,w) -> alpha (dtfactor * w - wsrc) + beta w = sourcew(u,q,w) 
//         // ->  (alpha * dtfactor + beta) w - alpha * wsrc - sourcew(u,q,w) = 0              
// 
//         // calculate the source term Sourcew(xdg, udg, odg, wdg)
//         HdgSourcewonly(s, s_wdg, xdg, udg, odg, wdg, uinf, physicsparam, time, modelnumber, ng, nc, ncu, nd, ncx, nco, ncs);            
// 
//         // alpha*dirkd/dt + beta
//         dstype scalar = common.dae_alpha*common.dtfactor + common.dae_beta;
// 
//         // calculate residual vector = sourcew(u,q,w) + alpha * wsrc - (alpha * dtfactor + beta) w
//         ArrayAdd3Vectors(s, s, wsrc, wdg, one, common.dae_alpha, -scalar, ng*ncs);                           
// 
//         // compute jacobian matrix = (alpha * dtfactor + beta) - s_wdg
//         ArrayAXPB(s_wdg, s_wdg, minusone, scalar, ng*ncs*ncs);                
// 
//         // solve the linear system jacobian * dw = residual
//         if (ncs==1) {                
//           SmallMatrixSolve11(s, s_wdg, ng, 1);
...
// 
//         // update w = w + dw
//         ArrayAXPBY(wdg, wdg, s, one, one, ng*ncs);
// 
//         // check convergence
//         dstype nrm = NORM(common.cublasHandle, ng*ncs, s, backend);
//         if (nrm < 1e-8) {     
//           // dwdu is actually s_udg 
//           HdgSourcew(s, dwdu, s_wdg, xdg, udg, odg, wdg, uinf, physicsparam, time, modelnumber, ng, nc, ncu, nd, ncx, nco, ncs);            
// 
//           // fix bug here
//           // compute jacobian matrix = (alpha * dtfactor + beta) - s_wdg
//           ArrayAXPB(s_wdg, s_wdg, minusone, scalar, ng*ncs*ncs);                
...
//           // w_udg = -inverse(s_wdg) * s_udg
//           if (ncs==1) {                
//             SmallMatrixSolve11(dwdu, s_wdg, ng, ncu);
//           }
//           else if (ncs==2) {
//             SmallMatrixSolve22(dwdu, s_wdg, ng, ncu);
//           }
//           else if (ncs==3) {
//             SmallMatrixSolve33(dwdu, s_wdg, ng, ncu);
//           }
//           else if (ncs==4) {
//             SmallMatrixSolve44(dwdu, s_wdg, ng, ncu);
//           }
//           else if (ncs==5) {
//             SmallMatrixSolve55(dwdu, s_wdg, ng, ncu);
//           }
//           else {
//             error("DAE functionality supports at most three dependent variables.");
//           }                  
//           
//           ArrayCopy(&wdg[ng*ncs], dwdu, ng*ncs*ncu);
//           
//           break;              
//         }
//         else {
...
// }
// 
// void extendedwEquation(dstype *wdg, dstype *wdg_udg, dstype *xdg, dstype *udg, dstype *odg, dstype *wsrc, 
//       dstype *tempg, appstruct &app, commonstruct &common, Int ng, Int backend)
// {        
//     Int ncu = common.ncu; // number of compoments of (u)
//     Int nd = common.nd; // spatial dimension
