...
void qoiElemBlock(solstruct &sol, resstruct &res, appstruct &app, ExasimDriverABI& driver_abi, masterstruct &master, 
...
{        
    ...
    QoIvolumeDriver(sg, xg, uqg, og, wg, driver_abi, mesh, master, app, sol, tmp, common, nge, e1, e2, backend);    
...
void qoiElement(solstruct &sol, resstruct &res, appstruct &app, ExasimDriverABI& driver_abi, masterstruct &master, 
...
{    
    ...
    for (Int j=0; j<common.nbe; j++) {              
        ...
        if (e2 <= common.ne1) qoiElemBlock(sol, res, app, driver_abi, master, mesh, tmp, common, common.cublasHandle, j, common.backend);        
    }                     
}

void qoiFaceBlock(solstruct &sol, resstruct &res, appstruct &app, ExasimDriverABI& driver_abi, masterstruct &master, 
        meshstruct &mesh, tempstruct &tmp, commonstruct &common, 
...
{            
    ...
    QoIboundaryDriver(tmp.tempn, &sol.faceg[nm], &tmp.tempg[nga*ncu], &sol.og1[ngf*nco*f1], 
            &tmp.tempg[nga*(ncu+nc)], &tmp.tempg[0], &sol.faceg[nm+n1], driver_abi, mesh, master, app, 
...
void qoiFace(solstruct &sol, resstruct &res, appstruct &app, ExasimDriverABI& driver_abi, masterstruct &master, 
        meshstruct &mesh, tempstruct &tmp, commonstruct &common)
{    
    ...
    for (Int j=0; j<common.nbf; j++) {
        ...
        if ((common.ibs > 0) && (ib == common.ibs))
            qoiFaceBlock(sol, res, app, driver_abi, master, mesh, tmp, common, common.cublasHandle, f1, f2, 1, common.backend);
    }                          
}
