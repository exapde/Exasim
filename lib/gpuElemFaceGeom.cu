...
template <typename T>
__global__ void  gpuTemplateFixNormal1D(T *nlg, int *facecon, int nga)
{
    int i = threadIdx.x + blockIdx.x * blockDim.x;
    while (i < nga) {        
        if (facecon[2*i]==0) nlg[i] = -1.0;
        i += blockDim.x * gridDim.x;
    }
}

template <typename T>
void gpuFixNormal1D(T *nlg, int *facecon, int nga)
{
    int blockDim = 256;
    int gridDim = (nga + blockDim - 1) / blockDim;
    gridDim = (gridDim>1024)? 1024 : gridDim;
    gpuTemplateFixNormal1D<<<gridDim, blockDim>>>(nlg, facecon, nga);
}
template void gpuFixNormal1D(double*, int*, int);
template void gpuFixNormal1D(float*, int*, int);
