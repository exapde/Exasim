...
static void FixNormal1D(dstype *nlg, Int *facecon, Int nga, Int backend)
{
#ifdef HAVE_ONETHREAD         
    if (backend == 0)       // One thread CPU
        opuFixNormal1D(nlg, facecon, nga);
#endif                   
#ifdef HAVE_OPENMP        
    if (backend == 1)  // Open MP
        cpuFixNormal1D(nlg, facecon, nga);
#endif                 
#ifdef HAVE_CUDA            
    if (backend == 2)  // CUDA C                
        gpuFixNormal1D(nlg, facecon, nga);
#endif                  
}
