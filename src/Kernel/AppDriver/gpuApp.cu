...
///// Source
template <typename T> __global__ void kernelgpuGradSourceEnzyme(T *f, T *df, T *xg, T *udg, T *dudg, T *odg, T *dodg, T *wdg, T *dwdg, T *uinf, T *param, 
        T time, int modelnumber, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw)
{    
...
template <typename T> void gpuSourceEnzyme(T *f, T *df, T *xg, T *udg, T *dudg, T *odg, T *dodg, T *wdg, T *dwdg, T *uinf, T *param, 
        T time, int modelnumber, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw)
{
	int blockDim = 256;
	int gridDim = (ng + blockDim - 1) / blockDim;
	gridDim = (gridDim>1024)? 1024 : gridDim;
	kernelgpuGradSourceEnzyme<<<gridDim, blockDim>>>(f, df, xg, udg, dudg, odg, dodg, wdg, dwdg, uinf, param, 
        time, modelnumber, ng, nc, ncu, nd, ncx, nco, ncw);
}

template void gpuSourceEnzyme(double *, double *, double *, double *, double *, double *, double *, double *, double *, double *, double *, 
        double, int, int, int, int, int, int, int, int);
