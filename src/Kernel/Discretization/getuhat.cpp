...
#ifdef HAVE_ENZYME
//// Method 1
void UhatBlockEnzyme(solstruct &sol, resstruct &res, appstruct &app, masterstruct &master, 
        meshstruct &mesh, tempstruct &tmp, commonstruct &common, cublasHandle_t handle, 
        Int nd, Int npe, Int npf, Int nc, Int ncu, Int ncx, Int nco, Int f1, Int f2, Int ib, Int backend)
{        
    Int ncw = common.ncw;
    ...
    if (ib==0) { 
        ...
        PutElemNodes(sol.uh, &tmp.tempn[0], npf, ncu, 0, ncu, f1, f2, backend);   
        //Update sol.duh = (duhat/du) v   
        PutElemNodes(sol.duh, &tmp.tempn[nn*ncu], npf, ncu, 0, ncu, f1, f2, backend);                  
    }
}

void GetUhatEnzyme(solstruct &sol, resstruct &res, appstruct &app, masterstruct &master, 
        meshstruct &mesh, tempstruct &tmp, commonstruct &common, cublasHandle_t handle, 
        Int nbf1, Int nbf2, Int backend)
{        
    Int nc = common.nc; // number of compoments of (u, q, p)
    ...
    for (Int j=nbf1; j<nbf2; j++) {
        ...
        Int ib = common.fblks[3*j+2];    
        // UhatBlock(sol, res, app, master, mesh, tmp, common, handle, f1, f2, ib, backend);
        UhatBlockEnzyme(sol, res, app, master, mesh, tmp, common,
                handle, nd, npe, npf, nc, ncu, ncx, nco, f1, f2, ib, backend);
    }                           
}
