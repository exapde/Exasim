...
{        
    ...
    if (ncw>0) {
        GetElemNodes(tmp.tempn, sol.wdg, npe, ncw, 0, ncw, e1, e2, backend);    
...
#ifdef HAVE_ENZYME       
//// Method 1
// void RuElemBlockEnzyme(solstruct &sol, resstruct &res, appstruct &app, masterstruct &master, 
//         meshstruct &mesh, tempstruct &tmp, commonstruct &common, cublasHandle_t handle, 
//         Int e1, Int e2, Int backend)
// {        
//     Int nc = common.nc; // number of compoments of (u, q, p)
...
//             ArrayAXY(&tmp.tempg[n8], &tmp.tempg[n8], &tmp.tempg[n5], one, nga*ncu, backend);                
//         }
...
//             ArrayAXPBY(&tmp.tempg[n8], &tmp.tempg[n8], &tmp.tempg[n7], one, one, nga*ncu, backend);            
//         }        
//     }
//     else {        
//         // calculate the source term Source(xdg, udg, odg, wdg)
...
//     // calculate sum_j Flux_j(u) * Xx(:,j,i)  at gauss points on element
//     // .       rg = sg, fg, Xx, jac
//     ApplyXx4(&tmp.tempg[n9], &tmp.tempg[n4], &tmp.tempg[n5], &sol.elemg[nm+n1], &sol.elemg[nm+n2],
//         nge, nd, ncu, ne, backend);
//     //(Source(u) + timesource, w)_K + (Flux(u), nabla w)_K = sum_j (Flux_j(u), dw/ dx_j)_K  = sum_j (jac Flux_j(u), dw/dxi_i dxi_i/dx_j)_T
//     // = dw/dxi_i * (sum_j Flux_j(u) * Xx(:,j,i)) 
//     Gauss2Node(handle, &res.Rue[npe*ncu*e1], &tmp.tempg[n9], master.shapegw, nge*(nd+1), npe, ncu*ne, backend);                
...
// void RuElemEnzyme(solstruct &sol, resstruct &res, appstruct &app, masterstruct &master, 
//         meshstruct &mesh, tempstruct &tmp, commonstruct &common,         
//         cublasHandle_t handle, Int nbe1, Int nbe2, Int backend)
// {    
//     for (Int j=nbe1; j<nbe2; j++) {
//         Int e1 = common.eblks[3*j]-1;
//         Int e2 = common.eblks[3*j+1];            
//         RuElemBlockEnzyme(sol, res, app, master, mesh, tmp, common, handle, e1, e2, backend);
//     }                     
// }
...
{        
    ...
    if (common.tdep) { // for time-dependent problem                
        ...
        // ArrayCopy(&tmp.tempg[n8], &tmp.tempg[n9], nga*ncu, backend);
        // ArrayMultiplyScalar(&tmp.tempg[n8], -common.dtfactor, nga*ncu, backend); 
        ArrayAXPBY(&tmp.tempg[n8], &tmp.tempg[n8], &tmp.tempg[n9], one, -common.dtfactor, nga*ncu, backend);
...
{            
    ...
    ApplyJac(&tmp.tempg[n3], &tmp.tempg[n8], &sol.faceg[nm+n2], nga, ncu, ngf, backend);    
...
////Method 1
// // Calculate Ruf = <fhat(xdg, uhat, udg, odg, nl), w>_F 
// void RuFaceBlockEnzyme(solstruct &sol, resstruct &res, appstruct &app, masterstruct &master, 
//         meshstruct &mesh, tempstruct &tmp, commonstruct &common, 
//         cublasHandle_t handle, Int f1, Int f2, Int ib, Int backend)
// {            
//     Int nc = common.nc; // number of compoments of (u, q, p)
...
//         &sol.og1[ngf*nco*f1], &sol.og2[ngf*nco*f1], &tmp.tempg[n5], &tmp.tempg[n11], &tmp.tempg[n7], &tmp.tempg[n13],
...
//                 &tmp.tempg[n5], &tmp.tempg[n11], &tmp.tempg[n3], &tmp.tempg[n9], &sol.faceg[nm+n1], mesh, master, app, 
//                 sol, tmp, common, ngf, f1, f2, ib, backend);        
//     }        
...
// void RuFaceEnzyme(solstruct &sol, resstruct &res, appstruct &app, masterstruct &master, 
//         meshstruct &mesh, tempstruct &tmp, commonstruct &common,
//         cublasHandle_t handle, Int nbf1, Int nbf2, Int backend)
// {    
//     for (Int j=nbf1; j<nbf2; j++) {
...
//         Int f2 = common.fblks[3*j+1];    
//         Int ib = common.fblks[3*j+2];    
//         RuFaceBlockEnzyme(sol, res, app, master, mesh, tmp, common, handle, f1, f2, ib, backend);
//     }                          
// }
#endif
