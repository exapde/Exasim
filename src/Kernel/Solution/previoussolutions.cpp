...
//         if (common.wave==1) { 
//             /* get solution from the previous timestep */   
//             ArrayCopy(sys.wprev, sol.wdg, N, backend);   
//             /* wtmp = (1-sum(c))*w */  
//             ArrayAXPB(sys.wtmp, sol.wdg, scalar, zero, N, backend);        
//         }
        
        if (common.ncw>0) {             
            /* get solution from the previous timestep */   
            ArrayCopy(sys.wprev, sol.wdg, N2, backend);   
            /* wtmp = (1-sum(c))*w */  
            ArrayAXPB(sys.wtmp, sol.wdg, scalar, zero, N2, backend);        
        }
    }
    else { //BDF
...
//             if (common.wave==1)
//                 ArrayCopy(sys.wprev1, sol.wdg, N, backend);
            if (common.ncw>0)
                ArrayCopy(sys.wprev1, sol.wdg, N2, backend);
        }
...
//                 /* get solution from the previous timestep */  
//                 ArrayCopy(sys.wprev2, sol.wdg, N, backend); 
//             }
            if (common.ncw>0) {
                /* get solution from the two previous timesteps */ 
                ArrayCopy(sys.wprev1, sys.wprev2, N2, backend);            
                /* get solution from the previous timestep */  
                ArrayCopy(sys.wprev2, sol.wdg, N2, backend); 
            }
        }                    
...
//                 /* get solution from the one previous timestep */     
//                 ArrayCopy(sys.wprev3, sol.wdg, N, backend); 
//             }
            if (common.ncw>0) {
                /* get solution from the three previous timesteps */    
                ArrayCopy(sys.wprev1, sys.wprev2, N2, backend);             
                /* get solution from the two previous timesteps */
                ArrayCopy(sys.wprev2, sys.wprev3, N2, backend); 
                /* get solution from the one previous timestep */     
                ArrayCopy(sys.wprev3, sol.wdg, N2, backend); 
            }
        }                    
    }
}
