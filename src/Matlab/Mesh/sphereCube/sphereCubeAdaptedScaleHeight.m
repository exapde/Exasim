function [p,t,dgnodes] = sphereCubeAdaptedScaleHeight(order, R0, R1, paramsMSIS, indicesMSIS, mass)

n = 16;
xH = r_scaleHeightMSIS(paramsMSIS,indicesMSIS,mass);
...
dim1 = (n+1)*(n+1);
dim2 = dim1*(m+1);
pc(3,:) = reshape(ones(dim1,1)*xH',[dim2,1]);
