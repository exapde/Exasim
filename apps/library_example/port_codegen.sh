...
# 3. main.cpp — emit the canonical 10-line template that delegates to
#    `exasim::run<GeneratedModel>(argc, argv)`. Idempotent: overwrites
#    any prior main.cpp, so the template can be evolved here and old
#    codegen apps catch up on their next port.
cat > "$CG/main.cpp" <<'CPPEOF'
...
CPPEOF
echo "[port] wrote main.cpp (3-line exasim::run<GeneratedModel> wrapper)"
