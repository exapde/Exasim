...
#include "../../../backend/Common/common.h"        // declaration of variables and structs
#include "../../../backend/Common/cpuimpl.h"       // interface to cpu functions
#include "../../../backend/Common/kokkosimpl.h"    // interface to kokkos functions
#include "../../../backend/Common/pblas.h"         // wrappers for blas libaries and MPI

#include "../../../backend/Discretization/discretization.hpp" // discretization class
#include "../../../backend/Preconditioning/preconditioner.hpp" // preconditioner class
#include "../../../backend/Solver/solver.hpp"                 // solver class
#include "../../../backend/Visualization/visualization.hpp"  //  visualization class
#include "../../../backend/PointLocator/pointlocator.hpp"  //  point locator class
#include "../../../backend/Solution/solution.hpp"             // solution class

#ifdef HAVE_SHARED_MODEL_LIB
#include "../../../backend/Preprocessing/preprocessing.hpp" // preprocessing class
#endif

// User PDE model — defines the Poisson 2D struct used to instantiate
// `CSolution<GeneratedModel>`. Pointwise math + hand-written Jacobians; no
// DSL, no autodiff. Routes through the templated kernels via the
// `exasim::*Driver<GeneratedModel>` path; libpdemodel.hpp ABI is bypassed.
...
//     printf("MPI rank %d using GPU %d and CPU %d on host %s\n", mpirank, device, cpu, hostname );
  ...
  {        
    ...
    CSolution<GeneratedModel>** pdemodel = new CSolution<GeneratedModel>*[nummodels];
    // initialize file streams
    ofstream* out = new ofstream[nummodels];    
            
    for (int i=0; i<nummodels; i++) {        
        if (mpiprocs0==0) {
          pdemodel[i] = new CSolution<GeneratedModel>(filein[i], fileout[i], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend, builtinmodelID);                 
        }
        else if (mpiprocs0 > 0) {
          if (mpirank < mpiprocs0) { 
            pdemodel[i] = new CSolution<GeneratedModel>(filein[0], fileout[0], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend, builtinmodelID);       
          }
          else {
            fileoffset = mpiprocs0;
            cout<<filein[1]<<endl;
            cout<<fileout[1]<<endl;
            pdemodel[i] = new CSolution<GeneratedModel>(filein[1], fileout[1], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend, builtinmodelID);       
          }
        }
        ...
        {            
            ...
            for (Int j=0; j<pdemodel[0]->disc.common.tstages; j++) {     
                ...
                for (int i=0; i<nummodels; i++) {
                    ...
                    // update solution 
                    UpdateSolution<GeneratedModel>(pdemodel[i]->disc.sol, pdemodel[i]->solv.sys, pdemodel[i]->disc.app, pdemodel[i]->disc.res, pdemodel[i]->disc.tmp, pdemodel[i]->disc.common, backend);
