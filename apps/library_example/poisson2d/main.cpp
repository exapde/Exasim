...
// `CSolution<Poisson2D>`. Pointwise math + hand-written Jacobians; no
// DSL, no autodiff. Routes through the templated kernels via the
// `exasim::*Driver<Poisson2D>` path; libpdemodel.hpp ABI is bypassed.
...
//     printf("MPI rank %d using GPU %d and CPU %d on host %s\n", mpirank, device, cpu, hostname );
  ...
  {        
    ...
    CSolution<Poisson2D>** pdemodel = new CSolution<Poisson2D>*[nummodels];
    // initialize file streams
    ofstream* out = new ofstream[nummodels];    
            
    for (int i=0; i<nummodels; i++) {        
        if (mpiprocs0==0) {
          pdemodel[i] = new CSolution<Poisson2D>(filein[i], fileout[i], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend, builtinmodelID);                 
        }
        else if (mpiprocs0 > 0) {
          if (mpirank < mpiprocs0) { 
            pdemodel[i] = new CSolution<Poisson2D>(filein[0], fileout[0], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend, builtinmodelID);       
          }
          else {
            fileoffset = mpiprocs0;
            cout<<filein[1]<<endl;
            cout<<fileout[1]<<endl;
            pdemodel[i] = new CSolution<Poisson2D>(filein[1], fileout[1], exasimpath, mpiprocs, mpirank, fileoffset, gpuid, backend, builtinmodelID);       
          }
        }
        ...
        {            
            ...
            for (Int j=0; j<pdemodel[0]->disc.common.tstages; j++) {     
                ...
                for (int i=0; i<nummodels; i++) {
                    ...
                    // update solution 
                    UpdateSolution<Poisson2D>(pdemodel[i]->disc.sol, pdemodel[i]->solv.sys, pdemodel[i]->disc.app, pdemodel[i]->disc.res, pdemodel[i]->disc.tmp, pdemodel[i]->disc.common, backend);
