function strkk = hdggencodefext2(filename, f, xdg, udg, odg, wdg, uhg, nlg, uext, tau, uinf, param, time, foldername)

strkk = "";
nbc = size(f,2);
for k = 1:nbc
    str1 = hdggencodeext2(filename + string(k), f(:,k), xdg, udg, odg, wdg, uhg, nlg, uext, tau, uinf, param, time);
    strkk = strkk + str1;    
end

cpufile = "Hdg" + filename;
tmp = "(dstype* f, const dstype* xdg, const dstype* udg, const dstype* odg, const dstype* wdg, const dstype* uhg, const dstype* nlg, const dstype* uext, const dstype* tau, const dstype* uinf, const dstype* param, const dstype time, const int modelnumber, const int ib, const int ng, const int nc, const int ncu, const int nd, const int ncx, const int nco, const int ncw)\n";
...
for k = 1:nbc
    ...
    tmp = tmp + "\t\t" + cpufile + string(k) + "(f, xdg, udg, odg, wdg, uhg, nlg, uext, tau, uinf, param, time, modelnumber, ng, nc, ncu, nd, ncx, nco, ncw);\n";
end
tmp = tmp + "}\n\n";
...
function strkk = hdggencodeext2(filename, f, xdg, udg, odg, wdg, uhg, nlg, uext, tau, uinf, param, time)
