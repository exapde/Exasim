...
void mkmaster(masterstruct &master, Int porder, Int *pgauss, Int *quadType, Int nd, Int elemtype, Int nodetype)
{
    ...
    master.proj_porder = proj_porder;
    ...
    masternodes(&master.plocvlP, &master.plocfcP, &master.proj_npv, &master.proj_npf, master.proj_porder, master.nd, master.elemtype, master.nodetype);
    Int proj_npv = master.proj_npv;
    Int proj_npf = master.proj_npf;
    ...
    mkshape(&master.shapvlP, &master.plocvlP[0], &master.gpvlQ[0], nqvQ, elemtype, proj_porder, nd, proj_npv);
    master.shapvtP.resize(proj_npv*nqvQ*(nd+1));
    master.shapvgP.resize(proj_npv*(nd+1)*nqvQ);
    master.shapvgdotshapvlP.resize(proj_npv*proj_npv*(nd+1)*nqvQ);
    for (i = 0; i < (nd+1); i++)
        for (j = 0; j < nqvQ; j++)
            for (k = 0; k < proj_npv; k++) {
                master.shapvtP[i*proj_npv*nqvQ+k*nqvQ+j] = master.shapvlP[i*nqvQ*proj_npv+j*proj_npv+k];
                master.shapvgP[i*nqvQ*proj_npv+j*proj_npv+k] = master.gwvlQ[j] * master.shapvlP[i*nqvQ*proj_npv+j*proj_npv+k];
            }
    for (i = 0; i < (nd+1); i++)
        for (j = 0; j < nqvQ; j++)
            for (k = 0; k < proj_npv; k++)
                for (l = 0; l < proj_npv; l++)
                    master.shapvgdotshapvlP[i*nqvQ*proj_npv*proj_npv + j*proj_npv*proj_npv + k*proj_npv + l] = master.shapvgP[i*nqvQ*proj_npv + j*proj_npv + l] * master.shapvlP[0*nqvQ*proj_npv + j*proj_npv + k];
