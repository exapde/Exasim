...
master.npv = master.npe;
master.ngv = master.nge;
master.nd = master.dim;
master.shapvt = master.shapegt;
master.shapvg = master.shapegw;
master.shapft = master.shapfgt;
master.shapfg = master.shapfgw;
master.shapmv = master.shapvt;
master.shapmf = master.shapft;

master.shapvgdotshapvl  = zeros(npe*npe,nge,dim+1);      
for d=1:dim+1
    for ii=1:npe
        for jj = 1:npe
            master.shapvgdotshapvl((ii-1)*npe+jj,:,d) = master.shapvg(jj,:,d).*shapeg(ii,:,1);                    
        end
    end            
end
...
for d=1:dim
    for ii=1:npf
        for jj = 1:npf
            master.shapfgdotshapfc((ii-1)*npf+jj,:,d) = master.shapfg(jj,:,d).*shapfg(ii,:,1);                    
        end
    end            
end
