function UDG1 = dgprojection(master1,mesh1,UDG,porder)
    ...
    dshapvt1  = reshape(permute(master1.shapent(:,:,2:nd+1),[1 3 2]),[ngv1*nd npv1]);
    plocvl = masternodes(porder,nd,master1.elemtype);
    shapt = mkshape(porder,plocvl,master1.gpe,master1.elemtype);
    ...
    for i=1:ne
        ...
        M1 = master1.shapen(:,:,1)*diag(master1.gwe.*jac1)*master1.shapen(:,:,1)';
        C1 = master1.shapen(:,:,1)*diag(master1.gwe.*jac1)*shap';
        L1 = C1*UDG(:,:,i);
        UDG1(:,:,i) = M1\L1;        
    end
