function [p,t] = refineaty_reverse(p,t,y)
...
% no refinement for y < y1 
[p1,t1] = removeelemement(p, t, ['y>' num2str(y1)]);

% yes refinement for y > y2
xw = 0.5*(xv(1:end-1)+xv(2:end));
xvref = sort([xv; xw]);
[p3,t3] = quadgrid(xvref,yv(i1+1:end));
...
for i = 1:nx    
    ...
    if rem(i,2)==1
        ...
        yi = [y1 y1 y2 y2 y2 0.5*(y1+y2) 0.5*(y1+y2)];        
        t2(:,:,i) = [1 7 5 4; 1 2 6 7; 7 6 3 5]+7*(i-1);
    else
        xi = [x1 x2 x2 x1 0.5*(x1+x2) x1 0.5*(x1+x2)];    
        yi = [y1 y1 y2 y2 y2 0.5*(y1+y2) 0.5*(y1+y2)];
        t2(:,:,i) = [1 2 7 6; 7 2 3 5; 6 7 5 4]+7*(i-1);
    end            
    p2(:,:,i) = [xi' yi'];
    m = m + 2;
end
