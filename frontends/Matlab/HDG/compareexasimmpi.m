function compareexasimmpi(master, mesh, pde, nproc, ne0)
...
for i = 1:nproc
  ...
  [MinvC1, MinvE1, ~, ~, uqg1, uhat1] = qEquationExasim(dataout, pde, npe, npf, nd, ne0);
  [AE1, FE1, DUDG1, DUDG_DUH1] = uEquationSchurExasim2(dataout, pde, npe, npf, nfe, ne0);  
  [Ru1, B1, D1] = uEquationElemExasim(dataout, pde, nge, npe, ne0);
  
  if i==1
    AE = squeeze(AE);
    AE1 = squeeze(AE1);    
    e = AE(:,:,[4 3 7 8]) - AE1(:,:,[1 2 3 4]);
    max(abs(e(:)))
    AE(:,:,4)-AE1(:,:,1)
    AE(:,:,8)
    AE1(:,:,4)
