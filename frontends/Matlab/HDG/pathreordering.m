function [fpath, lpath, fintf, lintf] = pathreordering(epath, nelems, e2f)
...
fpath = zeros(2,ne);
lpath = zeros(2,ne);
fintf = zeros(nfe-2,ne);
lintf = zeros(nfe-2,ne);

for i = 1:npaths
  ind = (nelems(i)+1):nelems(i+1);
  [fpath(:,ind), lpath(:,ind), fintf(:,ind), lintf(:,ind)] = pathreorder(epath(ind), e2f);
end
