function hdggencodeface(filename::String, f, xdg, udg, odg, wdg, uhg, nlg, tau, uinf, param, time, foldername)

    strkk = ""
    nbc = size(f, 2)
    for k in 1:nbc
        #str1 = hdggencodebou(filename * string(k), f[:, k], xdg, udg, odg, wdg, uhg, nlg, tau, uinf, param, time)
        str1 = hdggencodebou(filename * string(k), f[:, k], xdg, udg, odg, wdg, uhg, nlg, tau, uinf, param, time)
        strkk = strkk * str1
    end
