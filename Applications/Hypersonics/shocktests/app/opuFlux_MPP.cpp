template <typename T> void opuFlux(T* __restrict__ f, T* __restrict__ xdg, T* __restrict__ udg, T*__restrict__ odg, T*__restrict__ wdg, T*__restrict__ uinf, T*__restrict__ param, T time, int modelnumber, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw, Mutation::Mixture *mix)
{
	...
	for (int i = 0; i <ng; i++) {
		...
		T t29 = 1.0/t28;
		f[0*ng+i] = t29*udg6*(t23+3.182454300088011E-13)+odg1*udg8;//*(t18+(t2*t3)/((udg1*udg1)*1.0E+24+1.0));
		f[1*ng+i] = t29*udg6*(t24+3.182454300088011E-13)+odg1*udg9;//*(t19+(t2*t4)/((udg2*udg2)*1.0E+24+1.0));
		f[2*ng+i] = t29*udg6*(t25+3.182454300088011E-13)+odg1*udg10;//*(t20+(t2*t5)/((udg3*udg3)*1.0E+24+1.0));
		f[3*ng+i] = t29*udg6*(t26+3.182454300088011E-13)+odg1*udg11;//*(t21+(t2*t6)/((udg4*udg4)*1.0E+24+1.0));
		f[4*ng+i] = t29*udg6*(t27+3.182454300088011E-13)+odg1*udg12;//*(t22+(t2*t7)/((udg5*udg5)*1.0E+24+1.0));
		f[5*ng+i] = uinf1+odg1*udg13+t29*(udg6*udg6);
		f[6*ng+i] = odg1*udg14+udg6*(t29*udg7+t29*uinf1);
	}
