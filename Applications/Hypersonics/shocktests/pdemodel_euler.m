...
function avField = avfieldLaplacian(u, q, w, v, x, t, mu, eta, nspecies, nd)
    ...
    sb0   = mu(9); %0.02 
    sbmin = 0.0;
    sbmax = mu(10);% / sqrt(gam*gam - 1.0); %2.5 TODO: unsure that this should be changing 
    ...
    uv = ru.*r1;
    E = rE * r1; %energy
    p = gam1*(rE-r*0.5*(uv*uv));
    p = 1e-16 + lmax(p-1e-16,alpha);
    H = E + p*r1; %enthalpy
...
%     u0 = [rho_R_vec_nondim(:) + rho_L_vec_nondim(:); rhou_R_nondim + rhou_L_nondim; rhoE_R_nondim + rhoE_L_nondim];
    stepsize = 0.000001; 
    midpoint = 0.5;
    u0(1) = smoothstep_up(x-midpoint, rho_L_vec_nondim(1), rho_R_vec_nondim(1), stepsize);
    u0(nspecies+1) = 0.0;
