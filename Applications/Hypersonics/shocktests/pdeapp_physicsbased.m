...
pde.modelfile = "pdemodel_physicsbased";    % name of a file defining the PDE model
...
L_ref = 1.0;
rho_ref = 0.11566313067881752;
u_ref = 347.764810747843;
rhoE_ref = 13988.341078772399;
mu_ref = 1.9423965904207942e-05;
...
pde.externalparam(4) = T_ref;     % T_ref
...
mesh.vdg = zeros(size(mesh.dgnodes,1),4+nspecies,size(mesh.dgnodes,3));
mesh.vdg(:,4+nspecies,:) = (1.0 / (nDiv)) * ones(size(mesh.dgnodes,1), 1, size(mesh.dgnodes,3));
