...
% nt = 150
% dt = tfinal / nt;
dt = 2.5000000000000e-5;
%pde.dt = [dt*ones(1,50*nt),2*dt*ones(1, 50*nt), 5*dt*ones(1, 5*nt)]; %, 10*dt*ones(1, nt), 100*dt*ones(1,8*nt)];

pde.dt = [dt*ones(1,50*nt),2*dt*ones(1, 50*nt)]; %, 10*dt*ones(1, nt), 100*dt*ones(1,8*nt)];
%pde.dt = [dt*ones(1,50*nt), 2*dt*ones(1, 50*nt)];
% pde.dt = [dt*ones(1, nt)];
% tfinal = dt*nt + 10*dt*nt + 100*dt*8*nt
% pde.dt = [dt*ones(1,nt)];
pde.saveSolFreq = ceil(20*nt/20);
% pde.saveSolFreq = 100;
% pde.soltime = pde.saveSolFreq:pde.saveSolFreq:length(pde.dt); % steps at which solution are collected
%pde.timestepOffset = 5000;
...
rbody = 1;
rdomain = 3;
%[mesh.p,mesh.t,mesh.dgnodes] = mkmesh_circincirc_Ma17b(pde.porder,121,121,rbody,rdomain,1);
[mesh.p,mesh.t,mesh.dgnodes] = mkmesh_uniform(pde.porder,81,81,1,3,1);
...
%eval('!cp opuFbou_2d_inviscid_MPP.cpp opuFbou.cpp');
%eval('!cp opuFbou_2d_MPP.cpp opuFbou.cpp'); %TODO
%eval('!cp opuFlux_2d_visc_MPP.cpp opuFlux.cpp'); %TODO
%eval('!cp opuFbou_2d_visc_MPP.cpp opuFbou.cpp'); %TODO
...
%eval('!cp opuAvfield_2d_MPP_ducros.cpp opuAvfield.cpp');
%eval('!cp opuUbou_2d_inviscid_MPP.cpp opuUbou.cpp')
%eval('!cp opuUbou_2d_inv_test.cpp opuUbou.cpp');
eval('!cp ../app_inviscid_regen/opuFlux.cpp opuFlux.cpp'); %TODO
