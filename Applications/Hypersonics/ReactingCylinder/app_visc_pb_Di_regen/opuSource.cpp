template <typename T> void opuSource(T* __restrict__ f, T* __restrict__ xdg, T* __restrict__ udg, T*__restrict__ odg, T*__restrict__ wdg, T*__restrict__ uinf, T*__restrict__ param, T time, int modelnumber, int ng, int nc, int ncu, int nd, int ncx, int nco, int ncw, Mutation::Mixture *mix)
{
	...
	for (int i = 0; i <ng; i++) {
		...
		// T t4 = t2*t3;
		// T t5 = -t4;
		// f[0*ng+i] = t5+udg1*(t3*atan(param22*udg1)+1.0/2.0)+1.0/2.0;
		// f[1*ng+i] = t5+udg2*(t3*atan(param22*udg2)+1.0/2.0)+1.0/2.0;
		// f[2*ng+i] = t5+udg3*(t3*atan(param22*udg3)+1.0/2.0)+1.0/2.0;
		// f[3*ng+i] = t5+udg4*(t3*atan(param22*udg4)+1.0/2.0)+1.0/2.0;
		// f[4*ng+i] = t5+udg5*(t3*atan(param22*udg5)+1.0/2.0)+1.0/2.0;
		// f[5*ng+i] = 0.0;
		// f[6*ng+i] = 0.0;
		// f[7*ng+i] = 0.0;
	}
