...
% nt = 100000;
% pde.dt = 1e-4*ones(1,nt);   % time step sizes
% pde.saveSolFreq = 10000;
...
mesh.boundaryexpr = {@(p) abs(p(1,:)-a)<1e-8, @(p) abs(p(1,:) - b)<1e-8}; %TODO: double check boundaries
...
u = rhou./rho;
% subplot(1,3,3)
plot(dgnodes(:), u(:),'LineWidth',1.3); drawnow
% end
% plot
