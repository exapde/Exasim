function hh = meshplot_hr(mesh,opts,i_hr)
...
pars2={'facecolor',[1,0.5,0.5],'edgecolor','k','Linew',1,'FaceAlpha',1,'EdgeAlpha',1};
...
if opts(3)
    if dim == 2 || surface==1   
        for it=1:length(i_hr)
            pmid=mean(p(t(i_hr(it),:),:),1);
            txtpars={'fontname','times','fontsize',20,'horizontala','center','BackgroundColor',[1,1,1]};
%             text(pmid(1),pmid(2),num /2str(it),txtpars{:});
            hh=[hh;patch('faces',t(i_hr(it),:),'vertices',p,pars2{:})];
        end
    end
end
