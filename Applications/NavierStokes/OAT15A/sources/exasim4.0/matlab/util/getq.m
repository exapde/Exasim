function QDG = getq(master, mesh, UDG, UH, SH, fc_q)
...
for k=1:1:ne 
    ...
    QDG(:,:,k) = q; 
    if k==76
    reshape(permute(reshape(Cu,[npv nd nch]), [1 3 2]),[npv nch*nd])
    reshape(permute(reshape(Euh,[npv nd nch]), [1 3 2]),[npv nch*nd])
    end
end
