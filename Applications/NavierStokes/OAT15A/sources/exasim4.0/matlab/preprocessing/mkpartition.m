function [dmd,elem2cpu] = mkpartition(facecon,f,t2f,t2t,t,nproc,npe,bcm,perm)
...
for i = 1:nproc
    ...
    dmd{i}.facepart = faces(intf); % interior and exterior faces first  
    dmd{i}.facepartpts = length(intf); % 
    dmd{i}.facepartbnd = 0;
    ...
    if (sum(dmd{i}.facepartpts)-sum(dmd{i}.facepartpts(1)))~=length(bndf)
        error('something wrong');
    end
    %dmd{i}.nfbind = [0 0 0 0];     
...
%         [acolor,ncolor] = colorface(f,t2f,perm,faces((mf(j)+1):1:mf(j+1)));
