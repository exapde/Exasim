...
{    
    ...
    if (disc.common.RBcurrentdim>0) {
        ...
        ArrayAXPBY(sys.b, sys.u, sys.x, one, one, N, backend);  
        disc.evalResidual(sys.r, sys.b, backend);  
        dstype nrmr = PNORM(disc.common.cublasHandle, N, sys.r, backend);
        if (disc.common.mpiRank==0)
            cout<<"PTC Iteration: "<<0<<",  PTC Param: "<<disc.common.PTCparam<<",  Residual Norm: "<<nrmr<<endl;                           
...
Int PTCsolver(sysstruct &sys,  CDiscretization& disc, CPreconditioner& prec, Int backend)       
{
    ...
    dstype nrmr, tol, scalar, nrmrold, PTCparam;
    tol = disc.common.nonlinearSolverTol; // tolerance for the residual
                
    // keep the original value of disc.common.PTCparam
    PTCparam = disc.common.PTCparam;
    
    // evaluate the residual vector R(u) and set it to sys.r
    disc.evalResidual(sys.r, sys.u, backend);
        
    // compute the norm of the the residual vector
    nrmr = PNORM(disc.common.cublasHandle, N, sys.r, backend);
    //scalar = PNORM(disc.common.cublasHandle, N, sys.u, backend);        
    if (disc.common.mpiRank==0)
        cout<<"PTC Iteration: "<<it<<",  PTC Param: "<<disc.common.PTCparam<<",  Residual Norm: "<<nrmr<<endl;                                                    
    
    // check convergence
    if (nrmr < tol)
        return 0;
                
    // set up the PTC matrix B
    //TransientContinuationSetup(sys, disc, prec, backend);        
    ...
    for (it=1; it<maxit; it++) {                        
        ...
        LinearSolver(sys, disc, prec, backend);
        ...
        if (nrmr > 1.0e6) {                        
            ...
            if (disc.common.mpiRank==0)
                cout<<"Residual is NaN in file ptcsolver.cpp at line 258"<<endl;                           
...
//                 writearray2file(filename, disc.sol.udg, disc.common.ndofudg1, backend);       
//             
//             error("\nResidual norm in nonlinear solver is Inf.\n");
//         }
        ...
        // update the PTC parameter 
        UpdatePTC(sys, disc, prec, nrmrold, nrmr, backend);      
