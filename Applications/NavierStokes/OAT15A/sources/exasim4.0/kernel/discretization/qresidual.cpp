...
#include "../application/uhatDriver.cpp"
#include "../application/ubouDriver.cpp"
...
{            
    ...
    // ug = Phi*u -> nge*ne*ncu = (nge*npe)x(npe*ncu*ne)
    // ug*Xx -> nge*nd*ncu*nd*ne = (nge*ne*ncu)x(nge*ne*nd*nd) 
    // npe*ncu*nd*ne = (npe*nge*nd)*(nge*nd*ncu*nd*ne)
    // npe*nge*nd*ne = (npe*nge*nd)x(nge*ne*nd*nd) shape*Xx
    // shape*Xx*ug = (npe*nge*nd*ne)x(nge*ne*ncu) 
    
    // q = C*u -> npe*ncu*nd*ne = (npe*npe)x(npe*ncu)x(nd*ne)
    // q = E*uh
