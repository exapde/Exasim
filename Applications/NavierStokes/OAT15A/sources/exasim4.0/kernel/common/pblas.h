...
#ifdef USE_FLOAT  
    if (backend == 2)     
        CHECK_CUBLAS(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, ng, nn, np, 
            cublasOne, shapt, ng, un, np, cublasZero, ug, ng));    
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, ng, nn, np, 
            cublasOne, shapt, ng, un, np, cublasZero, ug, ng));
#endif        
#endif             
}
...
#ifdef USE_FLOAT  
    if (backend == 2)     
        CHECK_CUBLAS(cublasSgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, np, nn, ng, 
            cublasOne, shapg, np, ug, ng, cublasZero, un, np));    
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, np, nn, ng, 
            cublasOne, shapg, np, ug, ng, cublasZero, un, np));
#endif        
#endif             
}
...
{
    Int *ipiv, *info;
    CHECK(cudaMalloc(&ipiv, n * batchSize * sizeof(Int)));
    CHECK(cudaMalloc(&info,  batchSize * sizeof(Int)));     
    ...
    dstype **Ap_d;
    CHECK(cudaMalloc(&Ap_d,batchSize*sizeof(dstype *)));
    Ap_h[0] = A;
    for (Int i = 1; i < batchSize; i++)
      Ap_h[i] = Ap_h[i-1]+(n*n);
    CHECK(cudaMemcpy(Ap_d,Ap_h,batchSize*sizeof(dstype *),cudaMemcpyHostToDevice));
    
    dstype **Cp_h = (dstype **)malloc(batchSize*sizeof(dstype *));
    dstype **Cp_d;
    CHECK(cudaMalloc(&Cp_d,batchSize*sizeof(dstype *)));
    Cp_h[0] = C;
    for (Int i = 1; i < batchSize; i++)
      Cp_h[i] = Cp_h[i-1] + (n*n);
    CHECK(cudaMemcpy(Cp_d,Cp_h,batchSize*sizeof(dstype *),cudaMemcpyHostToDevice));
        
#ifdef USE_FLOAT        
    CHECK_CUBLAS(cublasSgetrfBatched(handle,n,Ap_d,n,ipiv,info,batchSize));    
    CHECK_CUBLAS(cublasSgetriBatched(handle,n,(const dstype **)Ap_d,n,ipiv,Cp_d,n,info,batchSize));
#else            
    CHECK_CUBLAS(cublasDgetrfBatched(handle,n,Ap_d,n,ipiv,info,batchSize));    
    CHECK_CUBLAS(cublasDgetriBatched(handle,n,(const dstype **)Ap_d,n,ipiv,Cp_d,n,info,batchSize));    
#endif            
...
#ifdef USE_FLOAT  
    if (backend == 2)     
        CHECK_CUBLAS(cublasSdot(handle, m, x, incx, y, incy, local_dot));    
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDdot(handle, m, x, incx, y, incy, local_dot));    
#endif        
#endif             
...
#ifdef USE_FLOAT  
    if (backend == 2)     
        CHECK_CUBLAS(cublasSdot(handle, m, x, incx, y, incy, dot));    
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDdot(handle, m, x, incx, y, incy, dot));    
#endif        
#endif                 
}
...
#ifdef USE_FLOAT  
    if (backend == 2)     
        CHECK_CUBLAS(cublasSgemv(handle, CUBLAS_OP_N, m, n, alpha, A, lda, x, incx,
                             beta, y, incy));        
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDgemv(handle, CUBLAS_OP_N, m, n, alpha, A, lda, x, incx,
                             beta, y, incy));
#endif        
#endif                     
}
...
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, m, n, k, 
            alpha, A, lda, B, ldb, beta, C, ldc));
#endif        
#endif                     
}
...
#ifdef USE_FLOAT  
    if (backend == 2)     
        CHECK_CUBLAS(cublasSgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, m, n, k, 
            alpha, A, lda, B, ldb, beta, Clocal, ldc));                    
#else            
    if (backend == 2)  
        CHECK_CUBLAS(cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N, m, n, k, 
            alpha, A, lda, B, ldb, beta, Clocal, ldc));
#endif        
#endif                     
