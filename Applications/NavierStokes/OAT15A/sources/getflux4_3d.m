function f = getflux4_3d(udg,odg,param)
gam = param(1);
gam1 = gam - 1.0;
Re = param(2);
Pr = param(3);
...
txx = Re1*c23*(2*ux - vy - wz) + (avb+avr)*(ux+vy+wz);
tyy = Re1*c23*(2*vy - ux - wz) + (avb+avr)*(ux+vy+wz);
tzz = Re1*c23*(2*wz - ux - vy) + (avb+avr)*(ux+vy+wz);
txy = Re1*(uy + vx);
txz = Re1*(uz + wx);
tyz = Re1*(vz + wy);
fv = [avr*rx, txx, txy, txz, uv*txx + vv*txy + wv*txz + (fc+(avb+avr)*gam)*Tx, ...
      avr*ry, txy, tyy, tyz, uv*txy + vv*tyy + wv*tyz + (fc+(avb+avr)*gam)*Ty, ...
      avr*rz, txz, tyz, tzz, uv*txz + vv*tyz + wv*tzz + (fc+(avb+avr)*gam)*Tz];
f = fi+fv;
