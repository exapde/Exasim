...
function flux(u, q, w, v, x, t, mu, eta)
    ...
    txy = Re1*(uy + vx);
    tyy = Re1*c23*(2*vy - ux);
    fv = [0, txx, txy, uv*txx + vv*txy + fc*Tx, 0, txy, tyy, uv*txy + vv*tyy + fc*Ty];
    f = fi+fv;
    f = reshape(f,(4,2));
...
function ubou(u, q, w, v, x, t, mu, eta, uhat, n, tau)
    ...
    uinf = mu[5:8]; # freestream flow
    uinf = uinf[:];
    u = u[:];          # state variables
    ui = 0.5*((u.+uinf) + An*(u.-uinf));  # Riemann solution
    uw = copy(u);
    uw[2] = 0; # zero velocity
    uw[3] = 0; # zero velocity

    ub = hcat(uw, ui); # wall and freestream boundary conditions
    return ub;
end
function fbou(u, q, w, v, x, t, mu, eta, uhat, n, tau)
    f = flux(u, q, w, v, x, t, mu, eta);
    fi = f[:,1]*n[1] + f[:,2]*n[2] + tau[1]*(u-uhat); # numerical flux at freestream boundary
    fw = copy(fi);
