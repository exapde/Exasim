...
template <typename T> 
__global__ void gpuTemplateGetElemFaceNodes(T *un, T *u, int *perm, int N, int nn, int ndf, int npe, int npf, int nc, int e1)
{        
...
template <typename T>
void gpuGetElemFaceNodes(T *un, T *u, int *perm, int npe, int npf, int nfe, int nc, int e1, int e2)
{    
    int ndf = npf*nfe;
    int nn = ndf*(e2-e1);    
    int N = nn*nc;
    ...
    gpuTemplateGetElemFaceNodes<<<gridDim, blockDim>>>(un, u, perm, N, nn, ndf, npe, npf, nc, e1);
}
template void gpuGetElemFaceNodes(float*, float*, int*, int, int, int, int, int, int);
template void gpuGetElemFaceNodes(double*, double*, int*, int, int, int, int, int, int);
